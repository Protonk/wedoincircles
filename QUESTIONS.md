...
Audit `corners/pairwise_non_inclusion_witnesses.sage`. The script claims to construct three witnesses for the iso/ hypothesis-class pairwise non-inclusions per `iso/THREE-REGISTER-SYNTHESIS.md` §3.2: thin ellipse `(a=2, b=1/2)` for Osserman ⊄ Fuglede; small-spike non-convex `r(θ) = 1 + ε cos(kθ)` with `k=5, ε=0.0575` for Fuglede ⊄ Osserman; and a categorial argument for Beck ⊄ {Osserman ∪ Fuglede}. Read the docstring's setup; check whether the script computes what it claims (especially the `elliptic_ec` perimeter convention, the polar-curvature formula and sign convention, and the volume-normalization scaling); verify the margin computations for the two numerical witnesses (witnesses 1 and 2); assess whether each sits cleanly inside/outside its hypothesis classes (margin ≥ 10 % audit threshold). **Witness 3 is excluded from numerical comparison** — it is a categorial type-signature argument with no numerical content; audit only its prose for structural soundness. Don't fix; report findings.
