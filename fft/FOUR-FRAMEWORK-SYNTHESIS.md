...
| 3DT / Ferenczi-Zamboni Lyndon words (candidate input) | Candidate machinery for the Kraft side: [rotations/3DT-BRIEF.md](rotations/3DT-BRIEF.md) §"Burrows-Wheeler and perfectly clustering Lyndon words" records a finite-word avatar of rotation-orbit structure via perfectly clustering Lyndon words. | Not one of the four FFT frameworks; not a bridge construction; no primitive-op lower bound or cyclotomic-cost theorem. The open question is whether the Lyndon/BWT avatar can become a prefix-free ledger coupled to `V_cert`'s certification costs. |
