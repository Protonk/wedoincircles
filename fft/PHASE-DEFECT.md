...
Located in the T4b ecosystem of [measure/FOR-BREAKFAST.md](measure/FOR-BREAKFAST.md) §K.6: T4b is the open load-bearing task of constructing a boundary object with `δ`-faithfulness for the bridge of [measure/THE-FIRST-BRIDGE.md](measure/THE-FIRST-BRIDGE.md). The candidate `δ ≡ cost of {Δ_k} compression` developed below is the candidate concrete coordinate for that task; PHASE-DEFECT's gating debts and four sub-debts are *the specific open conditions T4b decomposes into*, not separate program tasks orthogonal to the bridge. The substrate-side prototype is the σ-algebra coarsening theorem K (FOR-BREAKFAST §The kernel) on the integer-indexed lattice. The algorithm-side analog this memo would deliver is structurally non-trivial relative to that prototype: `δ` is continuum-valued, the closure class `C_FFT` is generated by FFT-style operations rather than by a discrete arithmetic projection, and the boundary state space requires construction rather than the atomic-σ-algebra inheritance the substrate side enjoys.
