...
**First test case (Schönhage-Strassen).** Verdict recorded in [fft/SCHOENHAGE-STRASSEN-COCYCLE-TRANSLATION.md](SCHOENHAGE-STRASSEN-COCYCLE-TRANSLATION.md): **conditional translation**. SS as a self-contained integer algorithm does not cross the additive/log-binade seam — there is no `ε` in the algorithm's primitive operations, and the `{Δ_k}`-compressibility question does not arise natively. SS-class methods are therefore *out-of-scope as native integer algorithms* and *in-scope only at floating-point interfaces*, where the cocycle question re-applies to the interface logic. The theorem scope narrows in a principled way: in-scope iff the algorithm crosses the seam at its primitive-operation level. This is *seam-orthogonal* scope-narrowing, distinct from the regularity-guard exclusion.
...
**Audit verdict.** Recorded in [fft/PHASE-LIFT-CONSERVATIVITY-AUDIT.md](PHASE-LIFT-CONSERVATIVITY-AUDIT.md): **(i) Clean.** Phase-lift conservativity does not import Baker, Gelfond-Schneider, or Thue-Siegel-Roth content. The natural recovery route — small-branch power series for `Arg(z)` around `z = 1`, geometrically convergent on `[0, ε_max]` — is pre-1882 calculus; per-sample cost is `O(1)` at fixed precision, giving competitive recovery. The H-L 1882 transcendence-at-dyadics fact remains the *input* to the bridge (Landfall §2 / §4), classified clean under content-not-calendar. **Caveat:** the bridge's contentfulness depends on `C_Aff` admitting bounded-degree polynomial approximations of fixed-precision real functions; under the natural extended reading (`C_Aff` = closure under machine arithmetic primitives) the bridge is true at competitive cost, under a strict `Aff⁺(ℝ)` reading the bridge is vacuous. The rewrite should make the `C_Aff` definition explicit.

## Sub-debts (under debt #2's expansion)

The cocycle reframe introduces four sub-debts:

- **Formal-character FFT composition.** Does FFT-style composition (the canon's standard composability) have a natural formulation at the formal-character level, or does it require analytic realization? If the latter, the cocycle reframe is an abstraction step, not a structural one.
...
**Gate status as of this revision.** Competitive compression has its operational definition (§"What this would buy"). Coordinate-cut alignment has its first canon test case at [fft/SCHOENHAGE-STRASSEN-COCYCLE-TRANSLATION.md](SCHOENHAGE-STRASSEN-COCYCLE-TRANSLATION.md): verdict **(b) Conditional** — theorem scope narrows to FFT-style methods that cross the additive/log-binade seam at their primitive-operation level. L-W audit on phase-lift conservativity at [fft/PHASE-LIFT-CONSERVATIVITY-AUDIT.md](PHASE-LIFT-CONSERVATIVITY-AUDIT.md): verdict **(i) Clean**. Per [fft/PHASE-DEFECT-PLAN.md](PHASE-DEFECT-PLAN.md)'s Promotion Rule, the rewrite is now *unblocked at the audit level*; whether the rewrite proceeds depends on the program's read of the conditional canon-translation verdict.

If quotient-clock alignment holds (under the seam-crossing scope), formal-cocycle non-compressibility argument is L-W clean (with phase-lift conservativity audited under content-not-calendar — done, clean), competitive compression is defined (done, operational form), and the four sub-debts close:
...
No Free Descent is **immune** to the broad-model lower-bound open problem Ailon flags, because transport is structurally different from proving a direct Fourier lower bound. The cocycle reframe claims that exporting FFT cheapness across the additive/log-binade seam must carry the defect cocycle; it does not claim an unrestricted linear-circuit lower bound. Ailon is therefore a scope marker and a proof-shape precedent for No Free Descent, not a threat to it. Add Ailon to References as adjacent FFT-side prior art when promotion lands.
