...
Optional follow-on to steps 1–2: **circumscribed counterpart**. ✓ Built. [corners/hurwitz_gap_circumscribed.sage](corners/hurwitz_gap_circumscribed.sage) does the tan-side calculation for the circumscribed regular $n$-gon ($L_n^{\text{circ}} = 2n \tan(\pi/n)$, $A_n^{\text{circ}} = n \tan(\pi/n)$). The closed form factors through the same bracket as the inscribed: $\Delta_n^{\text{circ}} = (L_n^{\text{circ}})^2 \cdot [1 - (\pi/n)\cot(\pi/n)]$, with ratio $\Delta_n^{\text{circ}} / \Delta_n^{\text{insc}} = \sec^2(\pi/n) \to 1$. Both sides converge to the common Archimedean leading term $4\pi^4/(3n^2)$ at the same polynomial rate. The Hurwitz Fourier support is identical ($m \equiv 1 \pmod n$); magnitudes scale by $|c_m^{\text{circ}}| = \sec(\pi/n) \cdot |c_m^{\text{insc}}|$, so the first-band concentration constants of [corners/HURWITZ-FIRST-BAND-CONCENTRATION.md](corners/HURWITZ-FIRST-BAND-CONCENTRATION.md) transfer verbatim — every paired-band $B_j$ and the gap $\Delta_n$ scale by the same $\sec^2(\pi/n)$ factor, so $B_1(n)/\Delta_n \to 6/\pi^2$ and the dyadic-shell estimate hold on the circumscribed side without modification. Companion figure: [figures/hurwitz_gap_archimedean_squeeze.png](figures/hurwitz_gap_archimedean_squeeze.png) (Archimedean squeeze in one panel; ratio panel verifies $\sec^2(\pi/n)$ to floating-point). Companion exposition: [corners/HURWITZ-GAP.md](corners/HURWITZ-GAP.md) §"Circumscribed counterpart and the Archimedean squeeze." The E-T-K $\times$ Aitchison Kraft budget that wants the inscribed $+$ circumscribed sum can now consume it numerically.
