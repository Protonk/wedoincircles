...
# Randomized-anchor variant of strip.py.
#
# Same mapping:
#   radial:  r in [1, sec(pi/3)=2] -> y = r - 1 in [0, 1]
#   angular: theta in [0, 2pi]     -> x = theta/(2pi) in [0, 1]
#
# Each n gets a random rotation phi_n ~ Uniform[0, 2*pi).
# n-gon edge k: r = sec(theta - 2*pi*k/n - phi_n)
#   spans theta in [2*pi*k/n + phi_n - pi/n, 2*pi*k/n + phi_n + pi/n]
#
# Cusps (tangent anchors) scatter across x instead of all sitting at x = k/n.
...
def plot(outpath, seed=SEED):
    ...
    ax.set_title("Archimedean strip, randomized anchors  |  n = 3 … 8", fontsize=12)
...
if __name__ == "__main__":
    ...
    out = os.path.join(figdir, "archimedean_strip_randomized.png")
    plot(out)
    print(f"wrote {out}")
