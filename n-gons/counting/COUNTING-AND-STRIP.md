...
The `X=\pm 1/2` contours are included for the same reason: they belong to the planar Niven/crystallographic value set. The all-`n` no-coincidence claim — that no integer pair `(n, k)` with `n \ge 3` lands on either contour — is **Lemma T1** of [measure/FOR-BREAKFAST.md](measure/FOR-BREAKFAST.md) §K.5, proved by squaring + trace + Ramanujan-sum reduction `4 c_n(2k+1) = \mu(n) - 3\varphi(n)` together with the `\varphi(h)` exclusion bound (`h = n/\gcd(n, 2k+1)`). The `\psi`-aligned table through `n = 30` and the wider numerical search through `n = 400` are consistent empirical witnesses; T1 supplies the proof. The visible counting backbone on the strip is therefore `X \in \{+1,0,-1\}`, with `X = \pm 1/2` provably empty across all `n \ge 3`.
...
The crystallographic comparison guides `X = \pm 1/2` are proved empty across all `n \ge 3` by Lemma T1; the figure shows the empirical *approach* behavior — `+1/2` is approached much more closely than `-1/2`. The asymptotic envelopes `\pi^2/(4n^2)` for the `n \equiv 3 \pmod 6` exact-cos-aligned subsequence and `\pi^2/(3 \cdot 6n)` for the rest give the *rate* of nearest approach. T1 rules out actual zero; the rate of approach is independent content recorded by the figure.
...
T1 of [measure/FOR-BREAKFAST.md](measure/FOR-BREAKFAST.md) §K.5 is the special case of the empty-non-backbone claim at `X = ±1/2` (no corner lands there for any `n \ge 3`). The broader collision-classification statement — same `(n_1, k_1), (n_2, k_2)` collision at any non-backbone `X` value — stays open. T1's proof technique (squaring identity + trace down to `\mathbb{Q}` + Ramanujan-sum reduction + `\varphi(h)` exclusion) is the natural tool to extend; whether it lifts to the full collision-classification theorem is the open question. T2 of [measure/FOR-BREAKFAST.md](measure/FOR-BREAKFAST.md) §K.5 is a parallel rigidity claim on the THICK-SWEEP thin-sweep coordinate that does *not* lift the same way (the slope `1/\sqrt{3}` adjoins `\sqrt{3} \in \mathbb{Q}(\zeta_{12})`, and the Ramanujan-sum reduction breaks); the analogous obstruction would need to be checked for any extension of T1 to the full collision classification.
