...
**Smarter-FFT rebuttal** (conditional on the above closing). A smarter FFT-style method does not answer the obstruction — same cost measures, same native operations, same closure classes, same `δ` at the boundary. The mathematical non-elimination claim is Lemma B's: smarter strategies redistribute or amortize cost but cannot zero `δ` at the boundary by finite composition. Coase 1937 (`measure/COASE-FRICTION-AND-SPECIALISTS.md`) supplies the vocabulary and methodological precedent — *reduce yes, eliminate no*, "Coasean specialists" — for naming what the lemma rules out. `δ` is on the substrate, not on the algorithm.
...
1. *Rotation-orbit Diophantine kinematics.* `β(π) = 0` places the substrate on the Diophantine side. Provenance audited L-W-safe in content per [rotations/BETA-PI-LW-AUDIT.md](rotations/BETA-PI-LW-AUDIT.md): the chain `finite μ(π) ⟹ β(π) = 0` terminates at Mahler 1953 / Hata 1993 / Salikhov 2008 (Hermite-Padé auxiliary-function arithmetic, no Baker / Gelfond-Schneider / Thue-Siegel-Roth content) plus classical CF algebra.
...
| 2 | Cost / conversion formalization | Foundation for `μ`, additive cost, adaptive conversion strategies, `δ` algebra, regimes, and `T(P)`. | Partly advanced: cocycle composition is typed in [fft/COCYCLE-COMPOSITION-LAW.md](fft/COCYCLE-COMPOSITION-LAW.md); amortization and asymptotics point to [memos/EFFECTIVE-HL-N1-COST-FORM.md](memos/EFFECTIVE-HL-N1-COST-FORM.md). Advice / non-uniformity is delegated to #9. | `measure/COASE-FRICTION-AND-SPECIALISTS.md`, `fft/PHASE-DEFECT.md`, `measure/ALGEBRA-OF-DELTA.md`, `memos/AMORTIZATION-AT-THE-BOUNDARY.md`, #9. | Do not say Lemma B closes the full impossibility region; it closes only the existence-side core. Do not treat advice as an algebra-of-`δ` sub-claim. |
...
| 7 | Trust-boundary discipline | Per-citation guardrail. | Ongoing. Each imported source needs its own "cited for / not cited for" boundary. | `fft/PROVENANCE-AND-TRANSFERABILITY.md`, `fft/AILON-2013-UNITARY-FFT-LOWER-BOUND-BRIEF.md`, `memos/BOWEN-DRILLING-AND-DENSITY.md`, `measure/COASE-FRICTION-AND-SPECIALISTS.md`. | Do not transfer theorem content from vocabulary or methodological precedents. |
| 8 | `β(π) = 0` L-W audit | Conditional support for Lemma A's first substrate-side bullet. | **Discharged (parallel-reading)** at [rotations/BETA-PI-LW-AUDIT.md](rotations/BETA-PI-LW-AUDIT.md): chain `finite μ(π) ⟹ β(π) = 0` is L-W-safe in content (Mahler 1953 / Hata 1993 / Salikhov 2008 are Hermite-Padé tradition; CF implication is pre-1882). | `memos/OLD-TIME-RELIGION.md`, [measure/SUBSTRATE-OBSTRUCTIONS.md](measure/SUBSTRATE-OBSTRUCTIONS.md), [rotations/BETA-PI-LW-AUDIT.md](rotations/BETA-PI-LW-AUDIT.md). | Do not re-import `β(π) = 0` into a QED-bearing slot — Lemma A is parallel-reading, and this audit closes the flag, not a QED obligation. |
