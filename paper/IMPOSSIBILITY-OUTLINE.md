...
**Why the non-transfer is structural, and what it lets §6 say.** The non-transfer is not an artifact of incomplete canon coverage; it is the empirical symptom of the algebraic-side closure-mismatch at NATIVE-F (§6.5). No closure-depth-preserving functor `F: C_log → C_circle` satisfying A1–A4 implies no canonical theorem-content can transport across the log/circle currency split — the canon's currencies are stratified by a closure-system divide that the integer-vs-continuum asymmetry of Theorem K mirrors on the substrate side. The non-transfer is also constructively witnessed inside the lower-bound technique landscape: Ailon 2013 (§3.7) demonstrates that Morgenstern's determinant potential cannot reach the normalized FFT (whose determinant has modulus 1) and matrix entropy is needed in its place — the currency-stratification is forced by what potentials can see, not a coverage gap. This upgrades the smarter-FFT rebuttal at §6.6 from posture to content: a smarter FFT-style method improving past `T(P)` would have to invent a new cross-currency or cross-regime transfer mechanism, and the impossibility theorem says FFT-style closure cannot manufacture one from the canon's native operations. The post-impossibility presentation lives at §7's multi-measure framing of `T(P)`.
...
**Algebraic-side companion** (§6.5). NATIVE-F's closure-mismatch theorem target asserts that no closure-depth-preserving functor `F: C_log → C_circle` satisfying axioms A1–A4 exists between the log-side and circle-side closure systems at competitive cost (closure generators `Aff` and `{K_n}_{n ≥ 3}`). Currently at proof-sketch status; awaits promotion.

**Conditional impossibility** (§6.6). Composes Theorem K (§5.6) + NATIVE-F-once-promoted (§6.5) + T4b with candidate transport (character reflection / phase-lift conservativity per `fft/PHASE-DEFECT.md`, §6.4) + endpoint commitment (§6.2). Construction debts: working ledger at end of document (outline-only; not paper content).
...
The closure-mismatch *theorem target* at [memos/NATIVE-F-MINIMAL-DEFINITION.md](memos/NATIVE-F-MINIMAL-DEFINITION.md) §No-Go Theorem asserts that no closure-depth-preserving functor `F: C_log → C_circle` satisfying axioms A1–A4 exists between the log-side and circle-side closure systems. Closure generators: `Aff` log-side (the affine class whose native-operation realization is `Aff⁺(ℝ)`); `{K_n}_{n ≥ 3}` circle-side, where `K_n = ℚ(cos(2π/n))` is the maximal real subfield of `ℚ(ζ_n)` (sometimes written `K_n^+`). Currently at proof-sketch status with promotion criteria stated; awaits closure-depth definitions and full promotion.

Substrate-side σ-algebra coarsening (Theorem K, §5.6) and algebraic-side closure-mismatch are two coordinates on the same integer-vs-continuum asymmetry — they block their respective descent channels (F-side coordinate reading, and routing through closure on the log-side). The algebraic-side block awaits NATIVE-F's promotion. **[Construction debt #6.]**

### §6.6. Conditional impossibility
Compose: (a) substrate-side facts — σ-algebra coarsening (Theorem K, §5.6) plus iso/-register currency-stratification at §5.2 (per §3.6.2 face (iv)); (b) algebraic-side closure-mismatch (NATIVE-F, §6.5, once promoted); (c) algorithm-side cost algebra with T4b boundary object plus candidate transport (§6.4); (d) endpoint commitment (§6.2).

Suppose `M` is an FFT-style method proving a lower bound on `P` strictly improving past `T(P)`. By (d), the descent implies `δ → 0` at the bounded/unbounded coefficient boundary (or competitive `{Δ_k}` compression in the candidate cocycle coordinate). (c) supplies the apparatus tying the cost coordinate to closure-class membership measurably and factors the endpoint through the substrate observables. (a) blocks two descent channels at once. *K's coarsening:* together with (c), F-side coordinate reduction cannot recover the three direct K2 substrate observables and therefore cannot supply the endpoint. *Iso/-register stratification:* together with (c), no descent that routes through free conversion between rate, constant, and almost-every readings of the planar isoperimetric gap supplies the endpoint either. T4b's broadened faithfulness (§6.4 (ii)) encodes the iso/-register currency structure into `δ`, so cross-register conversion costs read on `δ` alongside the algorithm-side `(μ, α)` cost; the endpoint's `δ → 0` therefore forces those conversion costs to vanish, contradicting the substrate-side fact that they are bounded below (the `5π` worked overhead between rate and constant, and the categorial type-gap to almost-every, are non-vanishing under any FFT-style passage). By (b) together with (c), no closure-depth-preserving functor on the log-side reaches the circle-side closure system at competitive cost, and therefore no descent routed through that closure supplies the endpoint.

Conditional on (b)–(d) closing — and on debt #9's currency reconciliation aligning `T(P)` with the uniform-charge variable-precision cost model — no FFT-style descent below current thresholds is reachable on this substrate.

The smarter-FFT rebuttal upgrades from posture to content via §3.6.2. Per §3.6.2, no canon source transfers a bound across another's coefficient regime or cost currency, and no FFT-style passage transfers between substrate-side iso/ registers without paying transaction cost; every such transfer is exactly `δ`. A smarter FFT-style method improving past `T(P)` would therefore have to invent a *new* cross-currency or cross-regime transfer mechanism on either side. The impossibility theorem — by composing (a)'s substrate-side facts (Theorem K's σ-algebra coarsening *plus* §5.2's iso/-register currency-stratification), (b)'s structural closure-mismatch, (c)'s T4b-plus-transport apparatus in each canon currency, and (d)'s endpoint commitment — says FFT-style closure cannot manufacture such a mechanism from the canon's native operations. The two substrate-side facts inside (a) work together: K rules out the *F-side coordinate channel* (recoding `(k, n) ∈ L` as a reduced rational `p/q ∈ F` and reading on `F` is forbidden the substrate observables `f₁, f₂, f₃`), while §5.2's iso/-register stratification rules out the *cross-register channel* (treating rate, constant, and almost-every readings of `Δ` as freely interconvertible is forbidden by the worked-instance overhead and the categorial type-gap, with each conversion a substrate-side `δ > 0` instance). Neither substrate-side channel supplies the endpoint, and (b)+(c)+(d) close the algebraic-side and algorithm-side channels symmetrically. The field's own survey aligns with this content claim: Ailon 2013 (§3.7) explicitly foregrounds that nontrivial broad linear-circuit Fourier lower bounds remain open, and known successful results require strong model restrictions — cross-currency or cross-regime transfer mechanisms are not in the literature. "Smarter FFT" therefore collapses to "FFT-style method plus machinery outside the canon's stack," which by §4.2's class definition puts the alleged method *outside* the FFT-style class, not inside being smarter. Coase 1937 (`measure/COASE-FRICTION-AND-SPECIALISTS.md`) supplies the *reduce yes, eliminate no* vocabulary; §3.6.2 supplies the content. The obstruction is structural, not in algorithmic cleverness. QED for §4 once (b)–(d) are earned and (9) reconciles.
...
| 6 | NATIVE-F closure-mismatch promotion | §6.5 | Open; awaiting closure-depth definitions and full promotion. Algebraic-side bookend; closes the affine-side-routed channel that Theorem K does not block. | [memos/NATIVE-F-MINIMAL-DEFINITION.md](memos/NATIVE-F-MINIMAL-DEFINITION.md) |
...
Earned and not on this list: σ-algebra coarsening (Theorem K, §5.6), Lemma T1 (off-backbone empty contour, [measure/FOR-BREAKFAST.md](measure/FOR-BREAKFAST.md) §K.5), T4a (common indexing domain `L`), the `β(π) = 0` L-W audit at [rotations/BETA-PI-LW-AUDIT.md](rotations/BETA-PI-LW-AUDIT.md). The destination is conditional on (1)–(6) plus (9)'s currency reconciliation; (7)–(8) and (10) are supports and discipline.
