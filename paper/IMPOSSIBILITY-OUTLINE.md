...
Figure: [figures/cost_conversion_schematic.png](figures/cost_conversion_schematic.png) — the cost-pair `(μ, α)` plane with the actual frontier (canon thresholds Morgenstern, AFW, Winograd marked), the counterfactual `δ = 0` frontier (the endpoint §4 aims to rule out), the hatched `δ`-gap between them, and the endpoint implication `descent past T(P) ⟹ δ → 0 at boundary` (§6.2) shown as a vertical arrow at the bounded/unbounded coefficient boundary (§1.4). Endpoint-side picture only; the cost algebra apparatus lives at §6.4 and the algebraic-side companion at §6.5. Companion at [paper/code/COST-CONVERSION-SCHEMATIC.md](paper/code/COST-CONVERSION-SCHEMATIC.md).
...
### §4.6. Proof outline
The proof has two parts plus a load-bearing transport, per `paper/SECOND-PROOF.md`.

**Substrate-side** (§5.6). Theorem K shows that the reduction `R: L → F` from the integer-indexed lattice `L = {(k, n) : 1 ≤ k < n, n ≥ 3}` to the Farey set induces σ-algebra coarsening on `L`, and three concrete substrate observables — cyclotomic-ladder degree `f₁ = φ(n)/2`, polygon perimeter `f₂ = L_n`, isoperimetric gap rate `f₃ = Δ_n` — are not recoverable from F-side data. *Proved here in companion form* (proof at `measure/FOR-BREAKFAST.md` §K.0–§K.4).

**Algorithm-side cost algebra** (§6.2–§6.4). The endpoint commitment ties threshold improvement to a cost-coordinate endpoint (§6.2: descent past `T(P)` implies `δ → 0`, equivalently competitive `{Δ_k}` compression in the candidate cocycle coordinate). The cost algebra apparatus types `δ` operationally with cocycle composition law and amortization conjecture (§6.4); the load-bearing open piece is **T4b**, a faithful boundary object with `δ`-coordinate satisfying `measure/THE-FIRST-BRIDGE.md`.

**Algebraic-side companion** (§6.5). NATIVE-F's closure-mismatch theorem target asserts that no closure-depth-preserving functor `F: C_log → C_circle` satisfying axioms A1–A4 exists between the log-side and circle-side closure systems (closure generators `Aff` and `{K_n}_{n ≥ 3}`). The statement is about *functors*, not about FFT-style methods; its operational scope is the narrow class of FFT-style descents that exhibit (or implicitly construct) such a closure-preserving correspondence. Sibling structural reading to K, not load-bearing for the main composition. Currently at proof-sketch status; promotion not required for the main impossibility.

**Conditional impossibility** (§6.6). Composes Theorem K (§5.6) plus iso/-register currency-stratification (§5.2) + T4b with candidate transport (character reflection / phase-lift conservativity per `fft/PHASE-DEFECT.md`, §6.4) + endpoint commitment (§6.2). NATIVE-F (§6.5) is named as a sibling structural reading but is not load-bearing here. Construction debts: working ledger at end of document (outline-only; not paper content).
...
Setup: descent in §1's framework means trading a higher cost-bearing complexity bound for a lower one by reorganizing the underlying computation. Lower-bound improvement *is* successful descent. The proof asks whether such descent is reachable by FFT-style methods past `T(P)`. Per §3.6.2's non-transfer, the §6 argument runs *currency-by-currency*: the endpoint commitment (§6.2), T4b (§6.4), and the candidate transport (§6.4) each land in every canon currency the impossibility is stated against. `δ` is conceptually single but realizes in a chosen cost-norm; the cost-algebra commitments are uniform in shape but plural-in-currencies in execution. The "currency-by-currency" quantification here is over *algorithm-side* canon currencies (`μ`, `α`); substrate-side iso/ currencies (§5.2, per §3.6.2's substrate-side currency-stratification face) feed in as facts — substrate-side `δ > 0` instances per §1.6's definition — not as additional axes §6 quantifies over.

### §6.2. Endpoint commitment
For descent past `T(P)` to succeed, the algorithm must drive `δ` at the bounded/unbounded coefficient boundary toward zero — which in the candidate cocycle coordinate of §1.7 reads as competitive compression of the per-sample `{Δ_k}` cost object. The endpoint commitment is the structural implication tying threshold improvement to the cost-coordinate's value at the boundary: descent past `T(P)` *implies* the endpoint condition; the implication direction is what `measure/THE-FIRST-BRIDGE.md` §What The Argument Must Show owes (a biconditional is not claimed). The endpoint is *stated* at the algorithm-side `(μ, α)` boundary, but with T4b's faithfulness extended to the §5.2 iso/-register currency structure (per §6.4), `δ → 0` on T4b automatically covers every `δ`-instance T4b's coordinate factors — algorithm-side and substrate-side iso/-register alike. The substrate-side `δ > 0` fact (per §3.6.2 face (iv)) then contradicts that endpoint, supplying the substrate-side currency-stratification half of the §6.6 composition's (a). **[Construction debt #5.]**

### §6.3. Substrate-side facts, algorithm-side consequences
**σ-algebra coarsening (Theorem K).** Theorem K (§5.6) certifies that an apparatus restricted to F-side / Stern-Brocot coordinates cannot recover the three direct K2 substrate observables `f₁, f₂, f₃`. Algorithm-side consequence: any candidate descent that operates by recoding `(k, n) ∈ L` as a reduced rational `p/q ∈ F` and reading on `F` is forbidden the cyclotomic-ladder degree, the polygon perimeter, and the isoperimetric gap rate. Theorem K does not by itself entail the working theorem — it blocks one specific channel (the F-side coordinate channel). The §6.4 cost algebra and §5.2's cross-register stratification (below) supply the rest.

**Iso/-register currency-stratification (§5.2).** §5.2 certifies that the planar isoperimetric gap admits three non-nesting measure-theoretic readings (rate, constant, almost-every) with positive worked-instance overhead between rate and constant (`5π` per `iso/THREE-REGISTER-SYNTHESIS.md` Claim 1) and a categorial type-gap to almost-every. Algorithm-side consequence: any candidate descent that routes through free conversion between these readings cannot zero `δ` — each conversion is a substrate-side `δ > 0` instance per §1.6's definition (per §3.6.2 face (iv)). §5.2 by itself does not entail the working theorem — it blocks one specific channel (the cross-register channel). T4b's broadened faithfulness at §6.4 encodes the iso/-register currency structure measurably so cross-register conversion costs read on `δ` alongside the algorithm-side `(μ, α)` cost; §6.6 composes the cross-register block with K's F-side coordinate block plus the endpoint commitment.

### §6.4. Cost algebra and candidate transport
The proof's algorithm-side commitment is that the conversion-cost coordinate `δ` admits an operational algebra inside a uniform-charge cost model.
...
**Amortization conjecture.** Whether per-instance specialization, table precomputation, or asymptotic averaging can drive per-instance `δ` below its existence-side floor is parked at [memos/AMORTIZATION-AT-THE-BOUNDARY.md](memos/AMORTIZATION-AT-THE-BOUNDARY.md). Open.
...
**Boundary object T4b.** A measure space `(Z, ℱ, μ)` together with a `δ`-coordinate satisfying the faithful-measurable-coordinate condition of [measure/THE-FIRST-BRIDGE.md](measure/THE-FIRST-BRIDGE.md), such that (i) the §5 scalar substrate-side observables `f₁, f₂, f₃` factor through `δ`, (ii) the §5.2 iso/-register currency structure (rate, constant, almost-every) is encoded measurably against `(Z, ℱ, μ)` so cross-register conversion costs read on `δ` alongside the algorithm-side `(μ, α)` cost, and (iii) closure-class membership reads measurably against `(Z, ℱ, μ, δ)`. The (ii) clause is the substrate-side currency-stratification piece of (a) in §6.6's composition; without it, the cross-register channel argument needs an external linkage between substrate-side and algorithm-side `δ`-instances. Open; load-bearing.

**Candidate transport.** Character reflection barrier with phase-lift conservativity as its analytic-exponential specialization, at [fft/PHASE-DEFECT.md](fft/PHASE-DEFECT.md). Carries the per-sample cost obstruction on `ε(m)` to a closure-class statement on `δ`. The original Landfall §2 affine-closure template no longer transfers at cost level under extended `C_Aff` (per [fft/LANDFALL-EXTENDED-CAFF-TRANSFER.md](fft/LANDFALL-EXTENDED-CAFF-TRANSFER.md)); the live source-side obstruction is Landfall §4 plus effective H-L. Open; candidate, not closed.

**[Construction debts #1, #2, #3, #4.]**
...
### §6.5. Algebraic-side companion (NATIVE-F closure-mismatch)
The closure-mismatch *theorem target* at [memos/NATIVE-F-MINIMAL-DEFINITION.md](memos/NATIVE-F-MINIMAL-DEFINITION.md) §No-Go Theorem asserts that no closure-depth-preserving functor `F: C_log → C_circle` satisfying axioms A1–A4 exists between the log-side and circle-side closure systems. Closure generators: `Aff` log-side (the affine class whose native-operation realization is `Aff⁺(ℝ)`); `{K_n}_{n ≥ 3}` circle-side, where `K_n = ℚ(cos(2π/n))` is the maximal real subfield of `ℚ(ζ_n)` (sometimes written `K_n^+`). Currently at proof-sketch status with promotion criteria stated.

**Scope.** The statement is about *functors* between two small observable categories, not about FFT-style methods. Its operational reading is narrow: it forecloses the subclass of FFT-style descents that exhibit (or implicitly construct) a closure-preserving correspondence between log-side affine apparatus and circle-side cyclotomic ladder. Descents that do not exhibit such a correspondence are not blocked by NATIVE-F.

NATIVE-F is therefore a sibling structural reading to Theorem K — both record the integer-vs-continuum asymmetry through a different formal language — and not part of the §6.6 main composition. The companion role is what NATIVE-F's own memo claims for itself ("structural rhyme, not theorem dependency", per `memos/NATIVE-F-MINIMAL-DEFINITION.md` §Methodological note); the present outline aligns with that scope. **[Construction debt #6: narrowed to companion-grade; promotion to closure-depth definitions remains, but is not required for the main impossibility.]**

### §6.6. Conditional impossibility
Compose: (a) substrate-side facts — σ-algebra coarsening (Theorem K, §5.6) plus iso/-register currency-stratification at §5.2 (per §3.6.2 face (iv)); (b) algorithm-side cost algebra with T4b boundary object plus candidate transport (§6.4); (c) endpoint commitment (§6.2). NATIVE-F (§6.5) is a sibling structural reading and not part of this composition.

Suppose `M` is an FFT-style method proving a lower bound on `P` strictly improving past `T(P)`. By (c), the descent implies `δ → 0` at the bounded/unbounded coefficient boundary (or competitive `{Δ_k}` compression in the candidate cocycle coordinate). (b) supplies the apparatus tying the cost coordinate to closure-class membership measurably and factors the endpoint through the substrate observables. (a) blocks two descent channels at once. *K's coarsening:* together with (b), F-side coordinate reduction cannot recover the three direct K2 substrate observables and therefore cannot supply the endpoint. *Iso/-register stratification:* together with (b), no descent that routes through free conversion between rate, constant, and almost-every readings of the planar isoperimetric gap supplies the endpoint either. T4b's broadened faithfulness (§6.4 (ii)) encodes the iso/-register currency structure into `δ`, so cross-register conversion costs read on `δ` alongside the algorithm-side `(μ, α)` cost; the endpoint's `δ → 0` therefore forces those conversion costs to vanish, contradicting the substrate-side fact that they are bounded below (the `5π` worked overhead between rate and constant, and the categorial type-gap to almost-every, are non-vanishing under any FFT-style passage).

Conditional on (b)–(c) closing — and on debt #9's currency reconciliation aligning `T(P)` with the uniform-charge variable-precision cost model — no FFT-style descent below current thresholds is reachable on this substrate.

The smarter-FFT rebuttal upgrades from posture to content via §3.6.2. Per §3.6.2, no canon source transfers a bound across another's coefficient regime or cost currency, and no FFT-style passage transfers between substrate-side iso/ registers without paying transaction cost; every such transfer is exactly `δ`. A smarter FFT-style method improving past `T(P)` would therefore have to invent a *new* cross-currency or cross-regime transfer mechanism. The impossibility theorem — by composing (a)'s substrate-side facts (Theorem K's σ-algebra coarsening *plus* §5.2's iso/-register currency-stratification), (b)'s T4b-plus-transport apparatus in each canon currency, and (c)'s endpoint commitment — says FFT-style closure cannot manufacture such a mechanism from the canon's native operations. The two substrate-side facts inside (a) work together: K rules out the *F-side coordinate channel* (recoding `(k, n) ∈ L` as a reduced rational `p/q ∈ F` and reading on `F` is forbidden the substrate observables `f₁, f₂, f₃`), while §5.2's iso/-register stratification rules out the *cross-register channel* (treating rate, constant, and almost-every readings of `Δ` as freely interconvertible is forbidden by the worked-instance overhead and the categorial type-gap, with each conversion a substrate-side `δ > 0` instance). The field's own survey aligns with this content claim: Ailon 2013 (§3.7) explicitly foregrounds that nontrivial broad linear-circuit Fourier lower bounds remain open, and known successful results require strong model restrictions — cross-currency or cross-regime transfer mechanisms are not in the literature. "Smarter FFT" therefore collapses to "FFT-style method plus machinery outside the canon's stack," which by §4.2's class definition puts the alleged method *outside* the FFT-style class, not inside being smarter. Coase 1937 (`measure/COASE-FRICTION-AND-SPECIALISTS.md`) supplies the *reduce yes, eliminate no* vocabulary; §3.6.2 supplies the content. The obstruction is structural, not in algorithmic cleverness. QED for §4 once (b)–(c) are earned and (9) reconciles.

## §7. Back again

Return to the four frameworks of §3 after the theorem closes. Schönhage–Strassen, Morgenstern, Winograd, AFW are then read as sitting at their natural limits — not from defect, but because no further descent is reachable on this substrate by these methods. Pair with the algebraic-side closure-mismatch reading (`memos/NATIVE-F-MINIMAL-DEFINITION.md`) as a sibling structural rhyme on the integer-vs-continuum asymmetry, not as a downstream piece of the impossibility. Figure re-reference: [figures/native_f_closure_mismatch.png](figures/native_f_closure_mismatch.png) — the §Intro panel re-read after the impossibility theorem closes; the closure-depth contrast is the structural payoff, not the setup.

**Multi-measure framing of `T(P)`.** The canon's lower-bound apparatus reads cost-of-`P` through heterogeneous measures, not a single `Ω(n log n)` shape: Morgenstern's `Ω(n log n)` bit-counting on bounded coefficients; Winograd's modular-product / CRT ledger `μ(T_P) = 2n − k` on bilinear-rational; AFW's multiplicative-complexity / cyclotomic-rational-equivalence ledger; Ailon 2013's Shannon-entropy potential on the restricted `2 × 2` unitary layered model. The Morgenstern↔Ailon pair is the canon's most explicit currency-stratification demonstration: same problem class (FFT), same scale (`Ω(n log n)`), two restricted models with two forced potentials — determinant on the unnormalized side, entropy on the normalized side (since determinant has modulus 1 there). This is the post-impossibility presentation of §3.6.2's non-transfer claim: the canon's currencies don't reconcile internally, and the impossibility theorem is what makes their plurality structurally meaningful rather than incidental. Cross-currency coherence with the substrate-side reading at Theorem K (§5.6) is the *intended* reconciliation; per debt #9, the reconciliation itself — both the canon re-read at variable precision and the cross-currency alignment of `T(P)` — is open, not established.
...
| [figures/delta_phase_plot.png](figures/delta_phase_plot.png) | [paper/code/build_delta_phase_plot.py](paper/code/build_delta_phase_plot.py) | [paper/code/COASE-PHASE.md](paper/code/COASE-PHASE.md) | §6.4 |
...
- **`memos/NATIVE-F-MINIMAL-DEFINITION.md`** *(in-program companion)* — closure-mismatch *theorem target* (no closure-depth-preserving functor `F: C_log → C_circle` satisfying A1–A4 between log-side and circle-side closure systems; closure generators `Aff` and `{K_n}_{n ≥ 3}`, where `K_n = ℚ(cos(2π/n))`). Currently at proof-sketch status with promotion criteria stated; awaits closure-depth definitions and full promotion (debt #6). Cited at §Intro, §6.5, and §7 as the algebraic-side companion.

## Substrate-side sources (§5)

- **`paper/MEASURE-THEORETIC-OBSTRUCTIONS.md`** *(paper-level pointer)* — routes the paper reader to the measure package and states the trust boundary: substrate-side typing supports Theorem K's σ-algebra coarsening reading at §5.6, not the cost-algebra apparatus (§6.4) or the conditional impossibility itself (§6.6).
...
> **Outline-only — not paper content.** Working ledger of construction debts that gate the §4.5 theorem. By the prose pass each debt will have been proven, resolved, or absorbed; this ledger does not survive into the paper. Inline `[Construction debt #N]` flags in §6.2, §6.4, §6.5 reference rows of the table below. Per-citation guardrails (trust boundaries, do-not-re-claim discipline) live at the linked anchor memos rather than in this table; consult those for the citation-by-citation contract.

The ten debts of `paper/SECOND-PROOF.md`, mapped to outline location, status, and anchoring memo.

| # | Name | Outline location | Status | Anchor |
|---|---|---|---|---|
| 1 | T4b — boundary object with `δ`-faithfulness | §6.4 | Open; central. The §5 scalar substrate-side observables and the §5.2 iso/-register currency structure factor through `(Z, ℱ, μ, δ)` (so cross-register conversion costs read on `δ` alongside the algorithm-side `(μ, α)` cost), and closure-class membership reads measurably. | [measure/THE-FIRST-BRIDGE.md](measure/THE-FIRST-BRIDGE.md), [fft/PHASE-DEFECT.md](fft/PHASE-DEFECT.md) |
| 2 | Algebra of `δ` and amortization conjecture | §1.6, §6.4 | Open. Cocycle composition is typed but not yet rigorously proved; amortization is open. | [fft/COCYCLE-COMPOSITION-LAW.md](fft/COCYCLE-COMPOSITION-LAW.md), [memos/AMORTIZATION-AT-THE-BOUNDARY.md](memos/AMORTIZATION-AT-THE-BOUNDARY.md), [measure/ALGEBRA-OF-DELTA.md](measure/ALGEBRA-OF-DELTA.md) |
| 3 | Effective H-L `n = 1` cost-form | §6.4 | Open; substrate-side delivery owed. Per-sample bound `cost_total ≥ c · p` against uniform-charge total cost on machine-dyadic `m = k/2^p`. | [memos/EFFECTIVE-HL-N1-COST-FORM.md](memos/EFFECTIVE-HL-N1-COST-FORM.md) |
| 4 | Character-reflection / phase-lift transport | §6.4 | Open; candidate, not closed. Carries the per-sample cost obstruction on `ε` to a closure-class statement on `δ`. | [fft/PHASE-DEFECT.md](fft/PHASE-DEFECT.md), [fft/LANDFALL-EXTENDED-CAFF-TRANSFER.md](fft/LANDFALL-EXTENDED-CAFF-TRANSFER.md) |
| 5 | Endpoint commitment | §6.2 | Open; load-bearing. Descent past `T(P)` implies `δ → 0` at the boundary (or competitive `{Δ_k}` compression in the candidate cocycle coordinate). | [measure/THE-FIRST-BRIDGE.md](measure/THE-FIRST-BRIDGE.md) §What The Argument Must Show |
| 6 | NATIVE-F closure-mismatch promotion | §6.5 | Open; sibling structural reading to K. Forecloses the narrow subclass of FFT-style descents that exhibit a closure-preserving correspondence between log-side and circle-side closure systems; not load-bearing for the §6.6 main composition. Promotion to closure-depth definitions still wanted but not required for §4.5. | [memos/NATIVE-F-MINIMAL-DEFINITION.md](memos/NATIVE-F-MINIMAL-DEFINITION.md) |
...
| 9 | Uniform-charge cost model, canon re-read, and `T(P)` currency reconciliation | §1.2, §6.4, §6.6, §7 | Methodological commitment set; canon re-read and currency reconciliation open. `T(P)` is currently specified outside the uniform-charge variable-precision model — currency reconciliation closes the alignment between threshold improvement and the `δ` cost-form obstruction. | [memos/COST-MODEL-UNIFORMITY-BRIEF.md](memos/COST-MODEL-UNIFORMITY-BRIEF.md), canon re-reads (Morgenstern, AFW, Winograd, Ailon) |
...
Earned and not on this list: σ-algebra coarsening (Theorem K, §5.6), Lemma T1 (off-backbone empty contour, [measure/FOR-BREAKFAST.md](measure/FOR-BREAKFAST.md) §K.5), T4a (common indexing domain `L`), the `β(π) = 0` L-W audit at [rotations/BETA-PI-LW-AUDIT.md](rotations/BETA-PI-LW-AUDIT.md). The destination is conditional on (1)–(5) plus (9)'s currency reconciliation; (6) is companion material whose promotion is wanted but not required for §4.5; (7)–(8) and (10) are supports and discipline.
