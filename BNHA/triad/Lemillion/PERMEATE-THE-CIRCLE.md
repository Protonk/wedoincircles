...
**Historical instance.** The son's method in `memos/BIDDER-AND-SON.md` is a 19th-century mental-arithmetic enactment of PERMEATE applied to log₁₀ computation: a small fixed set of universal scale constants (log of 1.001, 1.0001, 1.00001 plus the modulus and four prime logs), used by registering each target's contribution at every scale. The catalog is small, the procedure is fixed, the difficulty is bookkeeping rather than strategic choice. Useful as a worked example of what a PERMEATE saturation looks like in a problem domain other than the n-gon construction. The same memo flags the BIND-shaped closure question (which reals are exactly reachable from the son's constant set) that neither historical actor pursued.
