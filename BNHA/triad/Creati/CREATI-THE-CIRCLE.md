...
- **Reflection → circle side** (closed: no). Does trace on the circle admit a non-trivial reflection identity (some involution σ with trace(R_n) + trace(R_σ(n)) = closed form)? The transpose-trivial candidates fail; whether a richer reflection exists is an open enumeration question. **Verdict: no.** Enumeration at [BNHA/triad/Creati/CREATI-C3-REFLECTION-ENUMERATION.md](BNHA/triad/Creati/CREATI-C3-REFLECTION-ENUMERATION.md) shows no σ-involution on `n` produces a closed-form `f_σ` over an infinite domain; the unique 3-element witness `f_σ ≡ 0` on `{3, 4, 6}` for `σ(n) = 2n/(n−2)` is the equilateral-triangle/regular-hexagon coincidence, not a structural identity. The kind-mismatch is now a structural obstruction, not an open question.
