...
**Primitives used.** 2cos(2π/n) is an algebraic integer (the sum of a primitive n-th root of unity and its conjugate), closed-form via ζ_n + ζ_n^{-1}. Its minimal polynomial over ℚ has degree φ(n)/2 for n ≥ 3 (catalog item 9 in `BNHA/triad/Creati/CREATI-THE-CIRCLE.md`). An algebraic integer that is rational must be an integer (standard). An integer x satisfying |x| ≤ 2 is in {-2, -1, 0, 1, 2}.
...
**`n-gons/N-GON-WHOLENESS.md`.** The $\tau(n) = 0$ set is $\{1, 2, 3, 4, 6\}$ by Niven's theorem; this is worth stating explicitly in section §C1 (where $\tau$ is defined in the CREATI catalog of `BNHA/triad/Creati/CREATI-THE-CIRCLE.md`). What this note adds is not three independent closed proofs, but two independent algebraic proofs plus one saturation-first route that lands on the same set and exposes exactly where the tail has to be closed.

**`BNHA/triad/Creati/CREATI-THE-CIRCLE.md`.** The CREATI proof of Niven is literally catalog item 7 + catalog item 9 + the {-2,-1,0,1,2} enumeration. This can be added as a worked example of what a CREATI result looks like at the "closed-form result" admissible shape. Useful as a template.

**`BNHA/triad/Eraserhead/BIND-THE-CIRCLE.md`.** The Galois-orbit proof is the cleanest example of a BIND argument that stays entirely inside the algebraic-number-theory primitives without touching τ_c or the continuous-E tool. It's a reminder that BIND has discrete primitives available (Galois theory of cyclotomic fields) and isn't committed to analytic ones. Worth noting as a vocabulary check: when BIND uses Galois theory on ℚ(ζ_n), it's operating in primitives that are native to the circle-side algebraic structure but don't require importing anything from the log side. Erasure holds.
