...
**Witness.** `iso/THREE-REGISTER-SYNTHESIS.md` Claim 1 (proved): 5π overhead on chained Sobolev → geometric route, with no single extremal function realizing all three sharpnesses simultaneously. Convex restriction inherited from N_const (avoids Jordan; preserves Bonnesen content).
...
| `f_{rc}` specification + measurability + rescalings | ✓ Discrete-to-Borel into convex `N_const`; **`r_{rc}^{(κ)} = 5π` committed as the chain's structural cost commitment** (read on δ_Z via Phase 1b's rescaled-spread form); `r_{rc}^{(ν)}` deferred to natural choice |
